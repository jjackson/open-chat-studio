...
| `dragAxis`                   | `drag-axis`                     | Restrict dragging to a specific axis. Only applies when enableDragging is true.                                                                                                  | `"both" \| "x" \| "y"`                                                                     | `'both'`                                                                |
| `enableDragging`             | `enable-dragging`               | Enable dragging/moving the chat widget. When false (default), the widget position is fixed.                                                                                      | `boolean`                                                                                  | `false`                                                                 |
...
| `initialPosition`            | `initial-position`              | Initial position of the widget. Can be a corner name or pixel coordinates. Corners: 'bottom-right', 'bottom-left', 'top-right', 'top-left' Coordinates: { x: number, y: number } | `object \| string`                                                                         | `'bottom-right'`                                                        |
...
| `positionPersistence`        | `position-persistence`          | Custom persistence adapter for saving/loading widget position. Provide get() and set(pos) methods.                                                                               | `{ get: () => { x: number; y: number; }; set: (pos: { x: number; y: number; }) => void; }` | `undefined`                                                             |
| `safeMargin`                 | `safe-margin`                   | Safe margin from viewport edges in pixels. Can be a number or object with top/right/bottom/left.                                                                                 | `number \| object`                                                                         | `12`                                                                    |
| `snapThreshold`              | `snap-threshold`                | Distance in pixels from edge to trigger snap. Only applies when snapToEdges is true.                                                                                             | `number`                                                                                   | `32`                                                                    |
| `snapToEdges`                | `snap-to-edges`                 | Automatically snap to nearest edge when drag ends within snapThreshold pixels.                                                                                                   | `boolean`                                                                                  | `true`                                                                  |
...
| `zIndexOverride`             | `z-index-override`              | Override the z-index of the chat widget.                                                                                                                                         | `number`                                                                                   | `undefined`                                                             |
...
| ---------------- | --------------------------------------------------------------------- | ---------------------------------------- |
| `positionChange` | Event fired when widget position changes. Emits { x, y } coordinates. | `CustomEvent<{ x: number; y: number; }>` |
